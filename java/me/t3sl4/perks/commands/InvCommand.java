...
public class InvCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
        if(!(sender instanceof Player)) {
            ...
            World world;
            Location location;
            if (cmd.getName().equalsIgnoreCase("vipinv")) {
                if(sender.hasPermission("t3sl4perks.vipinv")) {
                    if(MessageUtil.DISABLE_WORLDS == true && MessageUtil.DISABLED_WORLDS.contains(((Player) sender).getWorld().getName())) {
                        ...
                        InvTime = MessageUtil.INV_TIME;
                        sender.sendMessage((MessageUtil.CMDINV).replaceAll("%time%", String.valueOf(InvTime)));
                        s = (Player)sender;
                        s.addPotionEffect(new PotionEffect(PotionEffectType.INVISIBILITY, 1200 * this.InvTime, 1));
                        if (!(MessageUtil.EFFECTS == true)) {
                            sender.sendMessage(MessageUtil.INVISIBILITY);
