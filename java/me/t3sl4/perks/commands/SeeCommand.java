...
public class SeeCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
        if(!(sender instanceof Player)) {
            ...
            World world;
            Location location;
            if (cmd.getName().equalsIgnoreCase("vipsee")) {
                if(sender.hasPermission("t3sl4perks.vipsee")) {
                    if(MessageUtil.DISABLE_WORLDS == true && MessageUtil.DISABLED_WORLDS.contains(((Player) sender).getWorld().getName())) {
                        ...
                        SeeTime = MessageUtil.SEE_TIME;
                        sender.sendMessage((MessageUtil.CMDSEE).replaceAll("%time%", String.valueOf(SeeTime)));
                        s = (Player)sender;
                        s.addPotionEffect(new PotionEffect(PotionEffectType.NIGHT_VISION, 1200 * SeeTime, 1));
                        if (!(MessageUtil.EFFECTS == true)) {
                            sender.sendMessage(MessageUtil.NIGHT_VISION);
